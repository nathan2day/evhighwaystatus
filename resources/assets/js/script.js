...
function initialise(){
    ...
    $(".connectors span").click(function(){
        connectorsController.update($(this).data("id"));
    });
...
function updateConnectorCheckmarks(){
    ...
    $(".connector").each(function(index,element){
        if (connectors.indexOf($(this).data('id')) >= 0)
        {
            $(this).find('i').fadeTo(0.5,1);
            return;
        }
        $(this).find('i').fadeTo(0.5,0);
    });
}
...
function initalGetRequest(){
    ...
    var url;
    if (isCordovaApp) {
        url = "https://evhighwaystatus.co.uk/";
    } else {
        url = "/"
    }
