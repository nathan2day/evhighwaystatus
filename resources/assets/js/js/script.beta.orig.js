...
function initialise(){
	...
	initalGetRequest();
	InitialiseMap();
	document.addEventListener('resume', providersController.refresh, false);
...
function setUp(){
	...
	if (GET["p"]!== undefined){
		providersController.update(GET["p"].toLowerCase());
	} else {
...
function AutoCompleteCustom(inputElement){
	...
	function addResult(prediction){
		...
		$resultRow = $("<div>",{class: "autocomplete-row"}).append('<div class="autocomplete-icon"><i class="fa fa-map-marker fa-fw"></i></div><div class="autocomplete-result">'+resultString+"</div>").click(function(e){
			$(inputElement).val(results.predictions[$(".autocomplete-row").index(e.delegateTarget)].description);
			//clearView();
	...
	function updateView(data){
		...
		if (!$(inputElement).val() || !$(document.activeElement).is(inputElement)) {
			clearView();
			return;
		}
...
function InitialiseMap() {
	...
	if (GET['z'] > 0 && GET['z'] < 21) {
		var zoom = parseFloat(GET['z']);
	} 

	if (GET['lng']> -12 && GET['lng']< 3 && GET['lat']> 48 && GET['lat']< 60) {
		if (GET['src'] == "evhw"){
			var center = {lat:parseFloat(GET['lat'])+0.02, lng:parseFloat(GET['lng'])};
		} else {
			var center = {lat:parseFloat(GET['lat']), lng:parseFloat(GET['lng'])};
		}
	...
	var acstartinput = document.getElementById('routestart');
	var acfinishinput = document.getElementById('routeend');
	...
	//autocompletestart = new google.maps.places.Autocomplete(acstartinput, acoptions);
	//autocompletefinish = new google.maps.places.Autocomplete(acfinishinput, acoptions);
	...
	function locationError(error){
		...
		switch(error.code) {
	        ...
	        case error.TIMEOUT:
	        	...
	        	if (locationOptions.enableHighAccuracy) {
	        		locationController.startWatch(false);
	        		return;
	        	}
...
var spinnerLoader = function(){
	...
	return {
		show: function(){
			...
			//$spinner = $("<div>",{class: "spinner"}).append($("<div>",{class: "cube1"}))
										   			//.append($("<div>",{class: "cube2"}));
			$container = $("<div>",{class: "spinner-container"});

			$(".wrap").after($container)
					  .after($spinner);
			//$("#mapbase").css({"opacity": "0.35"});
			spinnerActive = true;
...
var chargeSuggestionController = function(){
	...
	function addSuggestion(obj){
		...
		$row = $("<div>",{class: "charge-suggest-row w3-container w3-leftbar w3-pale-green w3-border-green"})
	...
	return {
		get: function(){
			...
			for (var i = 0; i < routeEnergyOverall.length; i++) {
				...
				if (closestCharger.info.connectors[0].status === "pseudo" || closestCharger.info.connectors[conNo].status === "online"){
		...
		processComments: function(response){
			var commentRequest = JSON.parse(response);
			...
			if (activeMarker.chargerHistory !== false){
				for (var i = 0; i < activeMarker.chargerHistory.length; i++) {
					...
					if (a.indexOf(activeMarker.chargerHistory[i].type) < 0){
						...
						content += '</td>';
						content += '<td' + statusColour(activeMarker.chargerHistory[i].new_status) + '>';
				...
				content += '<tr><td colspan="3">Looks like we don'+String.fromCharCode(39)+'t have any historical data yet..</td></tr>';
			}
...
function infoWindowContent (location){
	...
	if (location.provider === "Ecotricity"){
	...
	content += '<div class="w3-col s12 w3-right-align w3-padding-bottom"><span class="">Data source:</span> <span class=""><a href="'+location.source.url+'" target="_blank">'+location.source.name+'</a></spanLng></tr>';
	...
	'<span class="w3-btn w3-round w3-green w3-hover-blue w3-col s4 w3-border w3-border-white" onclick="openWindow.getHistoryInfoWindow.show()">History</span>'+
...
var providersController = function(){
	...
	return {
		update: function(provider,startup){
			//normal execution
			...
			if (startup === undefined){
				...
				var pLoc = providerreq.providers.indexOf(provider);

				if ( pLoc >= 0){
					providerreq.providers.splice(pLoc,1);
				} else {
					providerreq.providers.push(provider);
				}

				if(storagePossible) {
			    	localStorage.networks = JSON.stringify(providerreq);
				}
			} else {
				//first execution

				if (storagePossible && (localStorage.networks !== undefined)){
					providerreq = JSON.parse(localStorage.networks);
				} else {
					//providersController.selectAll();
				}
			...
			timeout = setTimeout(function(){
				submitJSONToServer("php/status_data.php",JSON.stringify(providerreq),loadStatusData);
			},750);
		...
		selectAll: function(){
			providerreq.providers = [];
			$(".networks span").each(function(index,element){
				providersController.update($(element).attr("data-value"));
			});
		},
		refresh: function(){
			submitJSONToServer("php/status_data.php",JSON.stringify(providerreq),loadStatusData);
...
function loadStatusData(response) {
	...
	var co = JSON.parse(response);

	if (response !== localStorage.data || firstRun){
		...
		if (GET['src'] == "evhw"){
			tweetLinkInfoWindow(GET["lat"],GET["lng"]);
		}
...
function updateUrl(o){
	if (!firstRun){
		...
		history.replaceState({},null,path+"?lat="+lat+"&lng="+lng+"&z="+z);
		if (o !== undefined){
			history.replaceState({},null,"/?lat="+lat+"&lng="+lng+"&z="+z +"&login=true");
		}
	}
}
...
function submitJSONToServer(target,jsondata,callback){
	var url;
	if (isCordovaApp) {
		url = "https://evhighwaystatus.co.uk/" + target;
	} else {
		url = target;
	}
	var xhttp = new XMLHttpRequest();
    ...
    xhttp.setRequestHeader('xAppAuth', '8;iLY3AZ1m7,?[pUKM0!+E7h44;u2W81dWl<(mf85kevN0J-MN^V6P1F47VTE77');
    xhttp.send(jsondata);
}
...
function cookieMessage(){
	...
	cookieMessage.style.cssText = "";
	var cookieNote = document.createElement("span");
	cookieNote.className = 'cookie-text';
	cookieNote.style.cssText = "";
	cookieNote.innerHTML = 'Just so you know, we use cookies to enhance your browsing experience and provide website statistics.';
	var cookieButton = document.createElement("button");
	cookieButton.className = "cookie-button";
	cookieButton.style.cssText = "";
	cookieButton.setAttribute("onClick","javascript:document.getElementsByClassName('cookie-message')[0].style.top = '100%';");
	cookieButton.innerHTML = 'Accept';
	var buttonHolder = document.createElement("span");
	buttonHolder.className = "cookie-button-container";
	buttonHolder.style.cssText = ''
	buttonHolder.appendChild(cookieButton);

	cookieMessage.appendChild(cookieNote);
	cookieMessage.appendChild(buttonHolder);

	document.getElementsByClassName("wrap")[0].appendChild(cookieMessage);
}
