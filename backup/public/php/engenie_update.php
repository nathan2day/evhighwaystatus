...
$response = json_decode(goFetch("http://api.openchargemap.io/v2/poi/?output=json&countrycode=GB&operatorid=203&maxresults=5000"),true);
...
if (count($response) > 1){
	...
	for ($i=0; $i < count($response) ; $i++) {
		...
		if ($highpower && $response[$i]["UsageTypeID"] != 2) {
			...
			$jsonData["locations"][$count]["provider"] = "Engenie";
			$jsonData["locations"][$count]["provider_openid"] = 203;
	...
	$myfile = fopen(dirname(__DIR__)."/json/engenie.json","w");
	fwrite($myfile, json_encode($jsonData));
	fclose($myfile);

	echo "Engenie: Success! ".count($jsonData["locations"]).' chargers updated.'.chr(10);
		
} else {
	echo 'Engenie: Update failed.'.chr(10);
}
