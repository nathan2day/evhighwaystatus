...
$response = goFetch("http://api.openchargemap.io/v2/poi/?output=json&countrycode=GB&operatorid=7&maxresults=5000");
...
if (count($response) > 20){
	...
	for ($i=0; $i < count($response) ; $i++) {
		...
		if ($highpower && $response[$i]["UsageTypeID"] != 2) {
			...
			$jsonData["locations"][$count]["provider"] = "Nissan";
			$jsonData["locations"][$count]["provider_openid"] = 7;
	...
	$myfile = fopen(dirname(__DIR__)."/json/nissan.json","w");
	fwrite($myfile, json_encode($jsonData));
	fclose($myfile);

	echo "Nissan: Success! ".count($jsonData["locations"]).' chargers updated.'.chr(10);
		
} else {
	echo 'Nissan: Update failed.'.chr(10);
}
