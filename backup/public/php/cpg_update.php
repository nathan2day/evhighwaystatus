...
$response = json_decode(goFetch("http://api.openchargemap.io/v2/poi/?output=json&countrycode=GB&operatorid=150&maxresults=5000"),true);
...
if (count($response) > 10){
	...
	for ($i=0; $i < count($response) ; $i++) {
		...
		if ($highpower && $response[$i]["UsageTypeID"] != 2) {
			...
			$jsonData["locations"][$count]["provider"] = "ChargePoint Genie";
			$jsonData["locations"][$count]["provider_openid"] = 150;
	...
	$myfile = fopen(dirname(__DIR__)."/json/cpg.json","w");
	fwrite($myfile, json_encode($jsonData));
	fclose($myfile);

	echo "CPG: Success! ".count($jsonData["locations"]).' chargers updated.'.chr(10);
		
} else {
	echo 'CPG: Update failed.'.chr(10);
}
