...
$response = goFetch("http://api.openchargemap.io/v2/poi/?output=json&operatorid=93&maxresults=5000");
...
if (count($response) > 20){
	...
	for ($i=0; $i < count($response) ; $i++) {
		...
		if ($highpower && $response[$i]["UsageTypeID"] != 2) {
			...
			$jsonData["locations"][$count]["provider"] = "eCar NI";
			$jsonData["locations"][$count]["provider_openid"] = 93;
	...
	$myfile = fopen(dirname(__DIR__)."/json/ecarni.json","w");
	fwrite($myfile, json_encode($jsonData));
	fclose($myfile);

	echo "ECarNI: Success! ".count($jsonData["locations"]).' chargers updated.'.chr(10);
		
} else {
	echo 'ECarNI: Update failed.'.chr(10);
}
