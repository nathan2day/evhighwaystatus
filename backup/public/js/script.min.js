...
c.targetChargers=[];
var firstRun=!0,heatmeupopen=!1,showingElevationInfo=!1,showingRouteSettings=!1,elevation,chart,elevResults=[],elevData,altInfoWindow,geocoder,storagePossible="undefined"!==typeof Storage?!0:!1,userController=function(){function a(a){g.getProfile(a,function(f,k){if(f)b(),g.show();else{providersController.update(!0);h=!0;localStorage.id_token=a;localStorage.userProfile=JSON.stringify(k);var x=k.user_metadata.firstName?k.user_metadata.firstName+" "+k.user_metadata.lastName:k.name;$(".username").html(x);
$(".logout").html("Logout");-1==k.picture.indexOf("cdn.auth0.com")&&$(".menu-icon-container").html('<img class="avatar" src="'+k.picture+'"></img>').addClass("profile-picture");k.email_verified?$(".username").html(x):($(".username").html(x+" (provisional)"),$(".username").css({color:"red"}),swal({title:"Welcome!",text:"Registration's complete. To unlock full functionality please verify your email address by following the link in the registration email.",type:"success",showConfirmButton:!0,confirmButtonText:""}));
...
g=$("<ul>",{"class":"models"}),k=0;a[b].models[k];)$(g).append($("<li>").append($("<span>").html('<i class="fa fa-check fa-fw" aria-hidden="true"></i>'+a[b].models[k].modelName).attr("value",String(b)))),k++;$(d).append(g);$(".manufacturers").append(d);b++}},setVehicle:function(g,f){chargeSuggestionController.setCapacity(1E3*a[g].models[f].battery.usable);$("#route-kwh-capacity").trigger("blur");b=g;d=f},getVehicle:function(){return{manufacturer:a[b].manufacturer,model:a[b].models[d]}}}}();
function AutoCompleteCustom(a){function b(a){var b=map.getCenter(),d=map.getBounds(),d=calcCordDistance(d.getNorthEast().lat(),d.getNorthEast().lng(),d.getSouthWest().lat(),d.getSouthWest().lng())/2;q=$(a).val();w={input:q,location:b.toUrlValue(),radius:1E3*d};ajaxHandler({url:"php/autocomplete.php",data:w,success:f})}function d(b){var d=b.description,f=d.substr(b.matched_substrings[0].offset,b.matched_substrings[0].length),g=d.substr(0,b.matched_substrings[0].offset);b=d.slice(b.matched_substrings[0].offset+
...
$("<i>",{"class":"material-icons"}).append("more_vert");if(arguments.length){var b=a.energy,b=b.toFixed(0)+"%",d=a.distance,d=d.toFixed(1)+"mi",f="transition-power";a.energy>v.getCapacity()-h&&(f+=" transition-power-exceeded");$row=$("<div>",{"class":"charge-suggest-transition"}).append([$("<div>",{"class":f}).text(d).css({width:"42%"}),$("<div>",{"class":"charge-transition-icon"}).append($img),$("<div>",{"class":f}).text(b).css({width:"42%"})])}else $row=$("<div>",{"class":"charge-suggest-transition"}).append([$("<div>").css({width:"42%"}),
$("<div>",{"class":"charge-transition-icon"}).append($img),$("<div>").css({width:"42%"})]);$(".charge-suggest-cont").append($row)}var g,f,h,k,w,q,x,D,n,I=[];(function(){var a,b,d,f;return function(g){b=a=0;for(d=!1;g<n.length&&!d;g++){a+=n[g].energy;b+=.621371*calcCordDistance(n[g].start.lat,n[g].start.lng,n[g].end.lat,n[g].end.lng);for(var h=0;h<k.length&&!d;h++)1>.621371*calcCordDistance(n[g].end.lat,n[g].end.lng,k[h].info.lat,k[h].info.lng)&&(d=!0,f=k[h])}return d?{type:"charger",energy:a,charger:f,
distance:b,location:{lat:f.info.lat,lng:f.info.lng}}:{type:"end",energy:a,location:{lat:n[g].end.lat,lng:n[g].end.lng}}}})();var v=function(){var a,b,d;return{getEnergyLevel:function(){return a},useEnergy:function(d){a-=d;b+=d},addEnergy:function(b){a+=b},setCapacity:function(a){d=a;localStorage.kwhCapacity=a/1E3;$("#route-kwh-capacity").val(Number(a/1E3).toFixed(1))},getCapacity:function(){return d},fillToLevel:function(b){a=b},fillToPercentage:function(b){a=v.getCapacity()*(b/100)},fillUp:function(){a=
d},fillToEightyPercent:function(){a=.8*d},getEnergyTrip:function(){return b},resetEnergyTrip:function(){b=0},isFull:function(){return a>=d}}}();return{get:function(a,b){var d=[];v.fillToPercentage(Number($("#route-kwh-start-capacity").val()));var m=updateMarkersOnMapFromRoute.getRouteMarkers();k=[];w=[];q=[];routeChargersSorted=[];g=directionsDisplay.getDirections();f=g.routes[curRoute];x=f.overview_path;for(var l=0;l<m.length;l++)k.push(m[l]),w.push(m[l]);for(l=0;l<k.length;l++)k[l].setOpacity(.4);
if(void 0!==a){processElevationData(a,b);if(b===google.maps.ElevationStatus.OK){var E=[],J=0;f.legs.forEach(function(a){J+=a.distance.value});f.legs.forEach(function(a){a.steps.forEach(function(a){E.push({step:a,ratio:a.distance.value/J})})});var K=1/a.length;a.forEach(function(b,d){var f,g=0,h=0;0<d?(f=b.elevation-a[d-1].elevation,0<f?g+=f:h+=Math.abs(f),descentCon=h/300*750,b.energy=g/300*1500-descentCon):b.energy=0});E.forEach(function(b){var d=0;for(b.elevData=[];d+K<=b.ratio;)b.elevData.push(a[0]),
a.splice(0,1),d+=K})}n=[];for(var y=[],t=0,A=0,p=0;p<f.legs.length;p++)for(var r=0;r<f.legs[p].steps.length;r++){stepEnergy=calcStepEnergyUsage(f.legs[p].steps[r]);var L=0;b===google.maps.ElevationStatus.OK&&E[A].elevData.forEach(function(a){L+=a.energy});for(m=1;m<f.legs[p].steps[r].path.length;m++){pathEnergy=stepEnergy/f.legs[p].steps[r].path.length;pathElevEnergy=L/f.legs[p].steps[r].path.length;y.push({lat:f.legs[p].steps[r].path[m].lat(),lng:f.legs[p].steps[r].path[m].lng()});var t=vehicleDataController.get().temperature,
D=20>=t?pathEnergy*(1+(20-t)/2*.01):pathEnergy*(1-(20-t)/4*.01),t=.621371*calcCordDistance(f.legs[p].steps[r].path[m-1].lat(),f.legs[p].steps[r].path[m-1].lng(),f.legs[p].steps[r].path[m].lat(),f.legs[p].steps[r].path[m].lng());n.push({energy:D+pathElevEnergy,start:{lat:f.legs[p].steps[r].path[m-1].lat(),lng:f.legs[p].steps[r].path[m-1].lng()},end:{lat:f.legs[p].steps[r].path[m].lat(),lng:f.legs[p].steps[r].path[m].lng()},distance:t,startStep:{lat:f.legs[p].steps[r].start_location.lat(),lng:f.legs[p].steps[r].start_location.lng()},
endStep:{lat:f.legs[p].steps[r].end_location.lat(),lng:f.legs[p].steps[r].end_location.lng()}});0==p&&0==r&&1==m&&k.push({info:{name:"Start",lat:f.legs[p].steps[r].path[m].lat(),lng:f.legs[p].steps[r].path[m].lng(),connectors:[{status:"pseudo"}]}});p==f.legs.length-1&&r==f.legs[p].steps.length-1&&m==f.legs[p].steps[r].path.length-1&&k.push({info:{name:"End",lat:f.legs[p].steps[r].path[m].lat(),lng:f.legs[p].steps[r].path[m].lng(),connectors:[{status:"pseudo"}]}})}A+=1}n[0].start.lat.toFixed(5);n[0].start.lng.toFixed(5);
n[n.length-1].end.lat.toFixed(5);n[n.length-1].end.lng.toFixed(5);var t=m=0,C=[],y=0;k.forEach(function(a,b){a.lastRouteBearing=0});for(l=0;l<n.length;l++){var m=m+n[l].energy,t=t+n[l].distance,y=y+n[l].distance,B=9999,u,z=0;k.forEach(function(a,b){z=calcCordDistance(n[l].start.lat,n[l].start.lng,a.info.lat,a.info.lng);z<B&&(0<routeChargersSorted.length?routeChargersSorted[routeChargersSorted.length-1].charger.info.name!==a.info.name&&(u=a,B=z):(u=a,B=z))});if(!(B>1.621371*$("#charger-radius").val())){if(l<
n.length-1){var M;nextDistanceFromHere=9999;z=0;k.forEach(function(a,b){z=calcCordDistance(n[l+1].start.lat,n[l+1].start.lng,a.info.lat,a.info.lng);z<nextDistanceFromHere&&(nextDistanceFromHere=z,M=a)});if(calcCordDistance(n[l+1].start.lat,n[l+1].start.lng,u.info.lat,u.info.lng)<B&&u.info.name===M.info.name)continue}var N=[];k.forEach(function(a,b){z=calcCordDistance(u.info.lat,u.info.lng,a.info.lat,a.info.lng);3>z&&0<z&&N.push(a)});if(1==N.length&&"End"!==u.info.name&&"Start"!==u.info.name){A=calcBearing(n[l].start.lat,
n[l].start.lng,n[l].end.lat,n[l].end.lng);p=calcBearing(n[l].start.lat,n[l].start.lng,u.info.lat,u.info.lng);bearing.minusDegrees(A,180);if(0!==u.lastRouteBearing&&90>bearing.smallestDelta(A,u.lastRouteBearing))continue;if(180<bearing.antiClockwiseDegrees(A,p)){u.lastRouteBearing=A;continue}}if(0<routeChargersSorted.length){var G=0;routeChargersSorted.forEach(function(a,b){a.charger.info.name===u.info.name&&0===G&&(G=a.distIntoRoute)});if(5>y-G)continue}if(!(0<routeChargersSorted.length&&"Start"===
u.info.name||0===routeChargersSorted.length&&"End"===u.info.name||0<routeChargersSorted.length&&"End"===u.info.name&&l!==n.length-1)){C=[];u.info.connectors.forEach(function(a){"pseudo"!==a.status&&C.push(a.type.id)});var H=-1;connectors.forEach(function(a){-1<C.indexOf(a)&&0>H&&(H=C.indexOf(a))});if("pseudo"===u.info.connectors[0].status||-1<I.indexOf(u.info.connectors[H].status))routeChargersSorted.push({charger:u,energy:m,distance:t,distIntoRoute:y}),"pseudo"!==u.info.connectors[0].status&&d.push({lat:u.info.lat.toFixed(5),
lng:u.info.lng.toFixed(5),startStep:{lat:n[l].startStep.lat.toFixed(5),lng:n[l].startStep.lat.toFixed(5)},endStep:{lat:n[l].endStep.lat.toFixed(5),lng:n[l].endStep.lat.toFixed(5)}}),t=m=0}}}for(l=y=t=m=d=0;l<routeChargersSorted.length;l++){m+=routeChargersSorted[l].energy;t+=routeChargersSorted[l].distance;0<l&&v.useEnergy(routeChargersSorted[l].energy);if(l<routeChargersSorted.length-1&&l==y){v.getEnergyLevel();routeChargersSorted[l].percentageOnArrival=v.getEnergyLevel()/v.getCapacity()*100;d=0;
A=y;for(p=0;p<routeChargersSorted.length;p++)if(p>l)if(0<l&&d+routeChargersSorted[p].energy+h<=.9*v.getCapacity()||0==l&&d+routeChargersSorted[p].energy+h<=v.getEnergyLevel())d+=routeChargersSorted[p].energy,y=p;else{A==y&&(y=l+1,d=.9*v.getCapacity()-h);break}v.getEnergyLevel();0<l&&v.fillToLevel(d+h);routeChargersSorted[l].percentageOnDeparture=v.getEnergyLevel()/v.getCapacity()*100;q.push({energy:m/v.getCapacity()*100,distance:t,suggestion:routeChargersSorted[l]});t=m=0}l==routeChargersSorted.length-
1&&(routeChargersSorted[l].percentageOnArrival=v.getEnergyLevel()/v.getCapacity()*100,q.push({energy:m/v.getCapacity()*100,distance:t,suggestion:routeChargersSorted[l]}))}t=q[0].suggestion.charger.info;directionsDisplay.getDirections();d=$("#routestart").val();t.name=d;t=q[q.length-1].suggestion.charger.info;directionsDisplay.getDirections();d=$("#routeend").val();t.name=d;t=[];for(l=1;l<q.length-1;l++)q[l].suggestion.charger.setOpacity(1),t.push({operator_id:q[l].suggestion.charger.info.provider_openid,
lat:q[l].suggestion.charger.info.lat,lng:q[l].suggestion.charger.info.lng});ajaxHandler({url:"php/OCM_comments.php",data:t,success:chargeSuggestionController.processComments});chargeSuggestionController.renderResult()}else elevation.getElevationAlongPath({path:x,samples:500},chargeSuggestionController.get)},setCapacity:function(a){v.setCapacity(a)},setStartTankLevel:function(a){v.fillToPercentage(a)},setLowLimit:function(a){h=v.getCapacity()/100*a},bounceMarkers:function(){a()},renderResult:function(){this.clearResult();
for(var a=0;a<q.length;a++)b(q[a]),a<q.length-1&&d(q[a+1]);$(".charge-suggest-cont").append($("<div>",{"class":"suggest-caution"}).text("Caution, this is currently in Beta. Check Plugshare before travel."));$(".charge-suggest-title").click(function(){var a=$.inArray(this,$(".charge-suggest-title"));0<a&&a<$(".charge-suggest-title").length-1&&q[a].suggestion.charger.showInfoWindow(!0)})},clearResult:function(){$(".charge-suggest-cont").children().remove()},resetOpacity:function(){if(w.length)for(var a=
...
else{$(g).append($("<span>").append("Comments powered by <a target='_blank' href='http://openchargemap.org/site/poi/details/"+a[b-1].comments[0].ChargePointID+"'>OpenChargeMap</a>"));for(var h,k,m=0;m<a[b-1].comments.length;m++){var n="bubble charge-suggest-comment"+commentType(a[b-1].comments[m].CheckinStatusTypeID),t=new Date(a[b-1].comments[m].DateCreated);m||(h=t,k=commentType(a[b-1].comments[m].CheckinStatusTypeID));var t=t.getDate()+" "+getActualMonth(t.getMonth())+" "+t.getFullYear().toString().substring(2,
4)+":",q="",p=a[b-1].comments[m].Comment,q=null===a[b-1].comments[m].CheckinStatusType?a[b-1].comments[m].CommentType.Title:a[b-1].comments[m].CheckinStatusType.Title;0<q.indexOf("(")&&(q=q.substring(0,q.indexOf("(")-1));if(p){var r=a[b-1].comments[m].UserName,r=r?" - "+r:"";g.append($("<div>",{"class":n}).append($("<div>",{"class":"comment-date"}).append(t)).append($("<div>",{"class":"comment-text"}).append(q)).append($("<div>",{"class":"user-comment collapsed-comment"}).append($("<div>",{"class":"quote-cont"}).append($("<i>",
{"class":"fa fa-quote-left"}))).append($("<span>").append(p)).append($("<div>",{"class":"quote-cont"}).append($("<i>",{"class":"fa fa-quote-right"}))).append($("<span>").append(r))))}else g.append($("<div>",{"class":n}).append($("<div>",{"class":"comment-date"}).append(t)).append($("<div>",{"class":"comment-text"}).append(q)).append($("<div>",{"class":"user-comment collapsed-comment"}).append($("<span>").append("No comment left..")).append($("<span>").append(r))))}b=daysBetweenDates(h,d);7<b?" comment-positive"==
