...
class Cyc
{
    ...
    {
        $this->getData();
        $locations = $this->parse();
	$provider = $this->chargers->providers->firstOrCreate(['name' => $locations[0]->provider,'url' => $locations[0]->source['url']]);
	foreach ($locations as $location){
            $charger = $provider->chargers()->firstOrCreate(['name'=> $location->name, 'lat' => $location->lat, 'lng' => $location->lng]);
	    foreach($location->connectors as $connector)
            {
		$existing = $charger->whereHas('connectors', function ($query) use ($connector) {
                    $query->where('name','=',$connector['type']['title']);
                })->count();

                if (!$existing) {
                    $charger->connectors()->attach(\App\Connector::where('name',$connector['type']['title'])->get(),['status' => $connector['status']]);
                }
            }
         }
    }
